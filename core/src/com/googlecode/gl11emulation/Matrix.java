 ...
 */
package com.googlecode.gl11emulation;

/**
 * Matrix math utilities. These methods operate on OpenGL ES 
 * format matrices and vectors stored in float arrays.
 * 
 * The only difference to the original Android class is that the
 * native methods are implemented in Java here.
 * 
...
public class Matrix {
    ...
    public static void multiplyMM(float[] ab, int abOfs, float[] a,
        ...
        float a0 = a[aOfs + 0], a1 = a[aOfs + 1], a2 = a[aOfs + 2], a3 = a[aOfs + 3];
        float a4 = a[aOfs + 4], a5 = a[aOfs + 5], a6 = a[aOfs + 6], a7 = a[aOfs + 7];
        float a8 = a[aOfs + 8], a9 = a[aOfs + 9], a10 = a[aOfs + 10], a11 = a[aOfs + 11];
        float a12 = a[aOfs + 12], a13 = a[aOfs + 13], a14 = a[aOfs + 14], a15 = a[aOfs + 15];
    ...
    public static void translateM(float[] m, int mOffset, float x, float y, float z) {
        m[12 + mOffset] += m[0 + mOffset] * x + m[4 + mOffset] * y + m[8 + mOffset] * z;
        m[13 + mOffset] += m[1 + mOffset] * x + m[5 + mOffset] * y + m[9 + mOffset] * z;
        m[14 + mOffset] += m[2 + mOffset] * x + m[6 + mOffset] * y + m[10 + mOffset] * z;
        m[15 + mOffset] += m[3 + mOffset] * x + m[7 + mOffset] * y + m[11 + mOffset] * z;
    }
