...
public class PlayerMovements {
    ...
    public static void PM_AirMove() {
        ...
        maxspeed = (pm.s.pm_flags & PlayerMove.PMF_DUCKED) != 0 ? pm_duckspeed
                : pm_maxspeed;
    ...
    public static void PM_CheckJump() {
        if ((pm.s.pm_flags & PlayerMove.PMF_TIME_LAND) != 0) {
            // hasn't been long enough since landing to jump again
            return;
        }

        if (pm.cmd.upmove < 10) { // not holding jump
            pm.s.pm_flags &= ~PlayerMove.PMF_JUMP_HELD;
            return;
        }
    ...
    public static void PM_CheckDuck() {
        ...
        if (pm.s.pm_type == Constants.PM_DEAD) {
            ...
            if ((pm.s.pm_flags & PlayerMove.PMF_DUCKED) != 0) {
                // try to stand up
                pm.maxs[2] = 32;
     ...
     */
    public static void Pmove(PlayerMove pmove) {
        pm = pmove;
